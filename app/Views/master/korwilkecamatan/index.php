        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="nama_korwil_kec" id="nama_korwil_kec" >
                        <small class="invalid-feedback"></small>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        const table = $("#table-data").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/master/korwilkecamatan/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                {
                    data: 'nama_korwil_kec',
                    name: 'nama_korwil_kec'
                },
               ...
               
                {
                    data: function(row) {
                           ...
                           let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.id_korwil_kec + '" data-nama_korwil_kec="' + row.nama_korwil_kec + '"data-id_korwil_prov="' + row.id_korwil_prov + '"data-id_korwil_kab="' + row.id_korwil_kab + '"data-id_provinsi="' + row.id_provinsi + '"data-id_kabupaten="' + row.id_kabupaten + '"data-id_kecamatan="' + row.id_kecamatan + '"><i class="fas fa-edit"></i></button>'
                           html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.id_korwil_kec + '"><i class="fa fa-trash"></i></button>'
                           return html;
                       }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/master/korwilkecamatan/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['id_korwil_prov','id_korwil_kab','id_provinsi','id_kabupaten','id_kecamatan','nama_korwil_kec'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#nama_korwil_kec").val($(this).data("nama_korwil_kec"));
            $(".modal-footer").append('<input type="hidden" name="id_korwil_kec" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/master/korwilkecamatan/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['id_korwil_prov','id_korwil_kab','id_provinsi','id_kabupaten','id_kecamatan','nama_korwil_kec'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/master/korwilkecamatan/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
