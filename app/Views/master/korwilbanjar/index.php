        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <select name="id_korwil_desa" id="id_korwil_desa" class="form-control">
                            <option value="">Pilih</option>
                            <?php foreach (esc($korwildesa) as $data) : ?>
                                <option value="<?= esc($data->id_korwil_desa) ?>"><?= esc($data->nama) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small class="invalid-feedback"></small>
                    </div>
                ...
                <div class="form-group row">
                    <label for="kategori" class="col-sm-4 col-form-label">Banjar</label>
                    <div class="col-sm-8">
                        <select id="id_banjar" name='id_banjar' class="form-control">
                            <option value="">Pilih</option>
                            <?php foreach (esc($banjar) as $data) : ?>
                                <option value="<?= esc($data->id) ?>"><?= esc($data->nama) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small class="invalid-feedback"></small>
                ...
                <div class="form-group row">
                    <label for="kategori" class="col-sm-4 col-form-label">Korwil Banjar</label>
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="nama_korwil_banjar" id="nama_korwil_banjar" >
                        <small class="invalid-feedback"></small>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        const table = $("#table-data").DataTable({
            ...
            order: [],
            ajax: {
                url: `${BASE_URL}/master/korwilbanjar/ajax`
            },
            //optional
            lengthMenu: [
                [10, 25,50,100],
            ...
            columns: [
                ...
                {
                    data: 'nama_korwil_banjar',
                    name: 'nama_korwil_banjar'
                },
                {
                    data: 'korwildesa',
                    name: 'korwildesa'
                ...
                
                {
                    data: function(row) {
                           ...
                           let html = '<button class="btn btn-success btn-sm mr-1 ubah" data-id="' + row.id_korwil_banjar + '"data-id_korwil_prov="' + row.id_korwil_prov + '"data-id_korwil_kab="' + row.id_korwil_kab + '"data-id_korwil_kec="' + row.id_korwil_kec + '"data-id_korwil_desa="' + row.id_korwil_desa + '"data-nama_korwil_banjar="' + row.nama_korwil_banjar + '"data-id_provinsi="' + row.id_provinsi + '"data-id_kabupaten="' + row.id_kabupaten + '"data-id_kecamatan="' + row.id_kecamatan + '"data-id_desa="' + row.id_desa + '"data-id_banjar="' + row.id_banjar + '"><i class="fas fa-edit"></i></button>'
                           html += '<button class="btn btn-danger btn-sm hapus" data-id="' + row.id_korwil_banjar + '"><i class="fa fa-trash"></i></button>'
                           return html;
                       }
                }
            ],
        ...
        $(".content").on("click", "#tambah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/master/korwilbanjar/tambah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                ...
                success: function(response) {
                    responValidasi(['tambah'], ['id_korwil_prov','id_korwil_kab','id_korwil_kec','id_korwil_desa','id_provinsi','id_kabupaten','id_kecamatan','id_desa','nama_korwil_banjar','id_banjar'], response);
                    if (response.sukses) {
        ...
        $(".content").on("click", ".ubah", function() {
            ...
            $("#id_korwil_desa").val($(this).data("id_korwil_desa"));
            ...
            $("#id_banjar").val($(this).data("id_banjar")); 
            $("#nama_korwil_banjar").val($(this).data("nama_korwil_banjar"));

            $(".modal-footer").append('<input type="hidden" name="id_korwil_banjar" value="' + $(this).data("id") + '">');
        })
        ...
        $(".content").on("click", "#ubah", function(e) {
            ...
            $.ajax({
                type: "post",
                url: `${BASE_URL}/master/korwilbanjar/ubah`,
                dataType: "json",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
                success: function(response) {
                    responValidasi(['ubah'], ['id_korwil_prov','id_korwil_kab','id_korwil_kec','id_korwil_desa','id_provinsi','id_kabupaten','id_kecamatan','id_desa','nama_korwil_banjar','id_banjar'], response);
                    if (response.sukses) {
                        $("#formModal").modal("hide");
                        table.ajax.reload();
                    }
        ...
        $(".content").on("click", ".hapus", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `${BASE_URL}/master/korwilbanjar/hapus`,
                        data: {
                            id: $(this).data("id")
                        },
                        success: function(response) {
